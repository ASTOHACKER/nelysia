...
| **`onError` position** | Applies only to routes declared after it — an `onError` at the end of the file misses route-level errors (validation falls back to the verbose default shape, plain `Error`s leak their message) | Applies to every route regardless of declaration order | When migrating, move `onError` before the routes on the Elysia side, or keep it at the end on the Nelysia side — both verified by runtime tests. |
