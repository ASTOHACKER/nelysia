...
const sourceFor = (mode) => {
  ...
  if (mode === "bun") return `${imports}${app}const server = app.listen(0); const response = await fetch("http://" + server.hostname + ":" + server.port + "/health"); if (response.status !== 200 || !(await response.json()).ok) throw new Error("Bun listen smoke failed"); server.stop?.(true); console.log("bun app.listen deployment smoke: PASS")\n`
